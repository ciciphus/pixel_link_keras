...
class PixelLinkNet(object):
    ...
    def _build_network(self):
        import config
        if config.model_type == MODEL_TYPE_mobile:
            from nets import mobile_net
            with slim.arg_scope([slim.conv2d],
                ...
                with slim.arg_scope([slim.conv2d, slim.max_pool2d],
                                    padding='SAME') as sc:
                    self.arg_scope = sc
                    self.net, self.end_points = mobile_net.mobilenet(
                        inputs=self.inputs,
                        is_training=False,
    ...
    def _fuse_by_cascade_conv1x1_upsample_sum(self, num_classes, scope):
        ...
        The score map is upsampled and added in a cascade way:
            start from the smallest score map, upsample it to the size
    ...
    def build_loss(self, pixel_cls_labels, pixel_cls_weights,
        ...
        with tf.name_scope('pixel_cls_loss'):
            ...
            def has_pos():
                # use pos_mask to calculate loss, neg_mask to  calculate weights
